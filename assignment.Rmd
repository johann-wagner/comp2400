...
```{r Potential Solution}
# All movies joined with total number of won director awards and total number
# of won actor awards.
MOVIE %>%
  
  left_join(
    {
      # Only the directors joined with total number of won director awards
      DIRECTOR %>% 
        inner_join({
          # Calculate total number of director awards for each movie
          # awarded in same year
          DIRECTOR_AWARD %>%
  ...
  left_join(
    {
      # Only the actors joined with total number of won actor awards
      ROLE %>% 
        inner_join({
          # Calculate total number of actor awards for each movie
          # awarded in same year
          ACTOR_AWARD %>%
            filter(str_to_lower(result) == "won") %>%
            group_by(title, production_year, description, year_of_award) %>%
            summarise(total_actor_awards = n())
        },
        join_by(title, production_year, description))
    },
    join_by(title, production_year)
  )
```
