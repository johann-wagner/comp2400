...
Which writers have won a writer award in 1994? List the ids, the first and last names of the writers along with the award names.

```{r Question 3}
WRITER_AWARD %>% 
  left_join(
    WRITER,
    join_by(id, title, production_year)
  ) %>%
  left_join(
    PERSON,
    join_by(id)
  ...
  select(id, first_name, last_name, award_name) %>% 
  kable()
```